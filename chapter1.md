...
* Они улучшают качество вашего кода, уменьшая количество ошибок.
...
* **Затем запускаем тест** - Тест должен выдать ошибку, поскольку пока не написан код, позволяющий ему выполниться без ошибок.
...
Введите в адресной строке браузера адрес локального сервера и URL, который мы назначили для доступа к нашему API \([http://127.0.0.1:8000/bucketlists\](http://127.0.0.1:8000/bucketlists%29\). Вуаля - все работает!
...
Введите в адресной строке браузера URL, который соответствует URL шаблону, связанному с DetailsView \([http://127.0.0.1:8000/bucketlists/1/\](http://127.0.0.1:8000/bucketlists/1/%29\). И вуаля - все работает! Теперь Вы можете отредактировать существующий список заветных желаний.

![](/images/part1/create2.png)

## Заключение

Наконец-то! Если Вы читаете это, значит Вы дошли до конца этой статьи - и Вы молодцы!

Во второй части мы рассмотрим вопросы, связанные с добавлением пользователей, интегрируем авторизацию и аутентификацию, задокументируем API и напишем более сложные тесты. Хотите узнать больше? Воспользуйтесь [официальной документацией](http://www.django-rest-framework.org/) DRF.

А если Вы новичок в Django, то это [пошаговое руководство](https://docs.djangoproject.com/en/1.10/intro/tutorial01/) прекрасно подойдет для Вас.

> Существует также перевод данного руководства на русский язык, доступный по адресу [http://djbook.ru/rel1.9/intro/tutorial01.html](http://djbook.ru/rel1.9/intro/tutorial01.html).

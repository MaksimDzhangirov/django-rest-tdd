...
Django содержит _диспетчер сигналов_. Диспетчер подобен приложению для обмена сообщениями в том смысле, что он оповещает всех о только что произошедшем событии. При создании пользователя, моделью `User` будет передан сигнал `post_save`. _Приемник_ \(который представляет собой просто функцию\) поможет нам перехватить этот `postsave` сигнал и сразу же создать токен.

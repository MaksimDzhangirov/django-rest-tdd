...
Эта новая строка добавляет DRF пути для аутентификации пользователя, используя шаблон для входа в систему по умолчанию. Вы можете выбрать любое другое название пути вместо `auth`.

Сохраните файл. При этом автоматически обновится рабочий сервер с учетом внесенных изменений.

Вы должны увидеть кнопку для входа в систему в правом верхнем углу экрана при переходе на страницу

```
 http://localhost/bucketlists/
```

Нажав на кнопку, произойдет переход на страницу входа в систему.

![](/images/part2/login.png)Давайте создадим суперпользователя и войдем в систему, используя его данные.

```
python3 manage.py createsuperuser
```

Войдем в систему, введя имя пользователя и пароль, которые мы только что указали.

## Авторизация: добавляем уровни доступа

Сейчас любой пользователь может просматривать и редактировать любой список заветных желаний. Мы хотели бы связать каждого пользователя со списками так, чтобы только владелец мог производить изменения, например, изменять или удалять его.

### Проверка уровня доступа, используемая по умолчанию
...
Класс `IsAuthenticated` отказывает в доступе к спискам любому не аутентифицированную пользователю и разрешает только аутентифицированным. Мы могли бы использовать класс `IsAuthenticatedOrReadOnly`, который позволяет не аутентифицированным пользователям использовать "безопасные" методы \(`GET`, `HEAD` и `OPTIONS`\) . Но чтобы повысить защиту приложения - мы будем использовать `IsAuthenticated`.

### Дополнительные уровни доступа

На данный момент любой аутентифицированный пользователь может просматривать списки заветных желаний других пользователей. Чтобы полностью реализовать такое понятие как "владелец списка", мы должны создать дополнительные уровни доступа.
...
Если мы выйдем из системы и попытаемся получить списки заветных желаний, вместо этого мы будет выдан `HTTP 403 Forbidden` ответ от сервера. Это означает, что наша аутентификация и авторизация действительно работают!

![](/images/part2/create4.png)

Превосходно!

Наконец мы запустим тесты и проверим успешно ли они пройдены:

![](/images/part2/test3.png)

Все тесты пройдены успешно.

## Как насчет аутентификации с использованием токенов?

Аутентификация с помощью токенов подходит для клиент-серверных приложений , особенно когда клиенты, использующие приложение, делают это с настольного компьютера или мобильного устройства.

Вот как это работает - Пользователь запрашивает криптографически защищенный токен \(токен аутентификации\) с сервера. Сервер генерирует  токен и связывает его с этим пользователем. После отправки токена пользователю, сервер ждет пока пользователь не запросит ресурсы, используя переданный ему токен. Пользователь может использовать токен для аутентификации и для подтверждения того, что он/она действительно является тем, кто имеет доступ к приложению.

Чтобы мы могли использовать аутентификацию с использованием токенов в нашем API, мы должны добавить некоторые настройки в файл `settings.py`.

Давайте добавим `rest_framework.authtoken` в наш список установленных приложений следующим образом:
...
Каждый раз при создании нового пользователя, мы бы также хотели бы создавать токен аутентификации для них. Но как нам гарантировать, что создание пользователя будет также инициировать создание токена?

Используя сигналы.

Django содержит _диспетчер сигналов_. Диспетчер подобен приложению для обмена сообщениями в том смысле, что он оповещает всех о только что произошедшем событии. При создании пользователя, моделью `User` будет передан сигнал `post_save`. _Приемник_ \(который представляет собой просто функцию\) поможет нам перехватить этот `post_save` сигнал и сразу же создать токен.

Наш приемник будет находится в файле `models.py`. В него мы импортируем сигнал `post_save`, модель `User`, используемую по умолчанию, модель `Token` и `receiver`:
...
Обратите внимание, что приемник расположен **НЕ ВНУТРИ КЛАССА-МОДЕЛИ Bucketlist**. Это довольно часто встречающаяся ошибка - располагать приемник внутри класса.
...
После сохранения файлов сервер автоматически повторно запустится с учетом добавленных изменений. Тем не менее, иногда нужно перезапустить сервер, используя команду `python3 manage.py runserver`.

Чтобы визуально проверить работу нашего API, создадим HTTP запросы в программе [Postman](https://www.getpostman.com/docs/introduction).

### Postman - 1 этап - получаем токен

Для аутентификации клиентов, полученный от сервера токен должен быть добавлен в HTTP заголовок `Authorization`. Введите слово Token, а затем через пробел сам токен. Заголовок должен выглядеть примерно следующим образом:

```
Authorization: Token 2777b09199c62bcf9418ad846dd0e4bbdfc6ee4b
```

Не забудьте о пробелах.

Чтобы получить сам токен, создадим POST запрос по адресу `http://localhost:8000/get-token/`, указав имя пользователя и пароль.

### ![](/images/part2/postman1.png)Postman - 2 этап - используем полученный токен в заголовке Authorization

При выполнении последующих запросов мы должны добавить заголовок `Authorization`, если хотим получить доступ к ресурсам API.

Ошибка, которая часто встречается на этом этапе, - это ввод заголовка `Authorization` в неправильном формате. Сообщение об ошибке от сервера обычно выглядит следующим образом:
...
Убедитесь, что Вы используете именно формат, указанный на первом этапе, а не - `Token - <сам-токен>`.  Если Вы хотите использовать другое ключевое слово в заголовке вместо `Authorization`, например, `Bearer`, просто создайте дочерний класс от `TokenAuthorization` и укажите это ключевое слово в виде свойства класса.

Давайте попробуем послать GET запрос. Вы должны получить примерно следующее:

![](/images/part2/postman2.png)

Теперь попробуйте поэкспериментировать с Вашим хорошо защищенным API.

## Выводы

Если Вы прочитали эту статью до конца - то Вы молодцы!

Мы рассмотрели довольно много! От реализации аутентификации пользователя до создания дополнительных уровней доступа при реализации авторизации , охватив таким образом большую часть тем, связанных с защитой Django API.

Мы также настроили удобный способ аутентификации, основанный на токенах, позволяющий клиентам с мобильных и настольных устройств, безопасно использовать наше API. Но что самое главное мы провели рефакторинг наших тестов, адаптировав их с учетом изменений. Это имеет первостепенное значение при разработке через тестирование.

Если при чтении у Вас возникает вопрос: "Что тут происходит?" я советую Вам посмотреть первую часть этой книги, которая является подробным пособием по созданию API для списка заветных желаний, используя TDD.

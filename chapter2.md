...
В этом сообщении говорится о том, что мы пытаемся добавить поле, значение которого не может быть равно NULL, но не указали какое значение нужно использовать. Таким образом, нужно указать значение для этого поля, которое будет вставлено в **уже существующие** строки таблицы. 
